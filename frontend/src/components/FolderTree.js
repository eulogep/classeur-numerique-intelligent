...
const FolderTree = ({ folders, onFolderSelect, selectedFolder, onFoldersChange, documents = [] }) => {
  ...
  return (
    <div className="folder-tree">
      ...
      {/* Stats globales en bas de sidebar */}
      <div className="sidebar-bottom-stats">
        ...
        <div className="sidebar-stat-row">
          <span>Dossiers</span>
          <span>{Object.keys(folders).length}</span>
        </div>
        <div className="sidebar-stat-row">
          <span>Stockage</span>
          <span>{(() => {
            const total = documents.reduce((s, d) => s + (d.size || 0), 0);
            if (total === 0) return '0 B';
            const i = Math.floor(Math.log(total) / Math.log(1024));
            return (total / Math.pow(1024, i)).toFixed(1) + ' ' + ['B','KB','MB','GB'][i];
          })()}</span>
        </div>
      </div>
    </div>
